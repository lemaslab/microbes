...
## Can't find this file ## COG_Pathway_File
# **************************************************************************** #
# ***************                   category2pathway.R                         #
# **************************************************************************** #
##Next steps:
...
## Step2: add a column to twin_wgs_L4_cog called RA (which is the average RA)
df$RA <- "blank"
for (i in 1:n) {df$RA[i] <- mean  (as.numeric(df[i,1:m])  )    }

cog.cv.ra <- as.data.frame (cbind(row.names(df), df$core_variable, df$RA))
names(cog.cv.ra ) <- c("cog.name", "core_variable","RA")
